...
public class ParseResultUtils {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(parseResultUtils.ESBUsageAnalysis("D:\\code\\demo-collection\\Service-Demo"));
    ...
    private static HashMap<String, SvcCallDetail> isESBUsageExist(HashMap<String, HashMap<String, Integer>> mappings) {
        ...
        for (Map.Entry<String, HashMap<String, Integer>> entry : mappings.entrySet()) {
            ...
            while (iterator.hasNext()){
                ...
                svcCallDetailHashMap.get(entry.getKey()).getCallService().add(map.getKey());
                if (!svcCallDetailHashMap.containsKey(map.getKey())){
                    svcCallDetailHashMap.put(map.getKey(),new SvcCallDetail());
                }
                svcCallDetailHashMap.get(map.getKey()).getCalledService().add(entry.getKey());
            }
        }
    ...
    private static void checkCallResult(HashMap<String, HashMap<String, Integer>> mappings) {
        Set<String> serviceNameSet = mappings.keySet();
        Set<String> lowercaseServiceNameSet = serviceNameSet.stream()
        ...
        for (Map.Entry<String, HashMap<String, Integer>> entry : mappings.entrySet()) {
            ...
            Iterator<Map.Entry<String,Integer>> iterator = call.entrySet().iterator();
            while (iterator.hasNext()){
                if (!lowercaseServiceNameSet.contains(iterator.next().getKey())){
                    iterator.remove();
                }
            }
        }
     ...
     * @return
     */
    public static HashMap<String, Integer> processMethods(HashMap<String,String> hashMap, ParseStorage parseStorage, String restTemplateName) {
...
//            parseRightExprs(callPathSetInit,parseStorage,i);
            ...
            System.out.println(callPathSetInit.toString());
            System.out.println("---------------");
            //针对字符串进行分割获取服务名
        ...
        if (node instanceof MethodCallExpr){
            ...
            if (((MethodCallExpr)node).getScope().isPresent()){
                ...
                if(restTemplateName.equals(callScope)){
                    ...
                    Expression arg0 = ((MethodCallExpr)node).getArguments().get(0);
//                    System.out.println(argStr.substring(argStr.indexOf("/"), argStr.lastIndexOf("/")));
                    callPathArgSet.add(arg0);
                }
            }
